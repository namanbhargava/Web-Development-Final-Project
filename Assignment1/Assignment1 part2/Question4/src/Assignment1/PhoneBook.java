...
class PhoneBook implements Serializable {
  public void addEntry(BookEntry entry) {
   String name = entry.getPerson().getSecondName();
    ...
    if(personBook.containsKey(name)) {       
  ...
  private void updateEntry(PhoneNumber number, BookEntry entry) {
  ...
  private void addNewEntry(PhoneNumber number, BookEntry entry) {
    LinkedList<BookEntry> list = new LinkedList<>();
    list.add(entry);
    numberBook.put(number, list);
  }
  ...
  public BookEntry[] getEntries(PhoneNumber number) {
    ...
    if(list == null){
      ...
      return list.toArray(new BookEntry[0]);                                                   
    }
  }
